...
class ChangePasswordView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"details": "Password has been changed successfully."}, status=status.HTTP_200_OK)
...
class RequestOTPView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Create a new OTP instance and send it
        ...
        if '@' in email_or_phone:
            ...
            send_mail(
                'Your OTP Code',
                f"Your OTP code is {otp_instance.otp_code}. It will expire in 5 minutes.",
                settings.EMAIL_HOST_USER,
                [user.email],
                fail_silently=False,
            )
        else:
            print(f"OTP for {user.phone_number}: {otp_instance.otp_code}")
        
        return Response({"details": "OTP has been sent to your email or phone."}, status=status.HTTP_200_OK)


class VerifyOTPAndSetPasswordView(generics.GenericAPIView):
    serializer_class = VerifyOTPAndSetPasswordSerializer
    permission_classes = [permissions.AllowAny]
    ...
    def post(self, request, *args, **kwargs):
        ...
        required_fields = ['email_or_phone', 'otp_code', 'new_password', 'confirm_password']
        missing_fields = [field for field in required_fields if not request.data.get(field)]

        if missing_fields:
            return Response({"details": f"The following fields are required: {', '.join(missing_fields)}"}, status=status.HTTP_400_BAD_REQUEST)
