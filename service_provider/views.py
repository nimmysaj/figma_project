...
from Accounts.models import AdManagement, ServiceProvider, ServiceRegister, ServiceRequest, User
from service_provider.permissions import IsOwnerOrAdmin
from .serializers import AdManagementSerializer, CustomerServiceRequestSerializer, InvoiceSerializer, ServiceProviderPasswordForgotSerializer, ServiceRegisterSerializer, ServiceRegisterUpdateSerializer, ServiceRequestSerializer, SetNewPasswordSerializer, ServiceProviderLoginSerializer,ServiceProviderSerializer
...
class AdManagementCreateView(generics.CreateAPIView):
    queryset = AdManagement.objects.all()
    serializer_class = AdManagementSerializer
    permission_classes = [permissions.IsAuthenticated]  # Ensures the user must be authenticated

    def perform_create(self, serializer):
        serializer.save(sender=self.request.user)  # Save the user who is creating the ad