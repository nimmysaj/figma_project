...
class Franchisee(models.Model):
    ...
    profile_image = models.ImageField(upload_to='f-profile_images/', null=True, blank=True,
                                      validators=[validate_file_size])  # Profile image field
...
class Dealer(models.Model):
    ...
    about = models.TextField()
    profile_image = models.ImageField(upload_to='d-profile-images/', null=True, blank=True,
                                      validators=[validate_file_size])  # Profile image field
    service_providers = models.IntegerField(blank=True, null=True)
...
class ServiceProvider(models.Model):
    ...
    address_proof_file = models.FileField(upload_to='id-service-pro/', blank=True, null=True,
                                          validators=[validate_file_size])  # File upload for address proof
...
class Customer(models.Model):
    ...
    profile_image = models.ImageField(upload_to='c-profile-images/', null=True, blank=True,
                                      validators=[validate_file_size])
    date_of_birth = models.DateField(null=True, blank=True)
    gender = models.CharField(max_length=1, choices=GENDER_CHOICES)
...
class CustomerReview(models.Model):
    ...
    customer = models.ForeignKey(User, on_delete=models.PROTECT,
                                 related_name='from_review')  # The customer leaving the review
    service_provider = models.ForeignKey(User, on_delete=models.PROTECT,
                                         related_name='to_review')  # The service provider being reviewed
...
class ServiceRequest(models.Model):
    ...
    acceptance_status = models.CharField(max_length=20,
                                         choices=[('accept', 'accept'), ('decline', 'decline'), ('pending', 'pending')],
                                         default='pending')
...
class Complaint(models.Model):
    ...
    service_request = models.ForeignKey(ServiceRequest, on_delete=models.SET_NULL, null=True, blank=True,
                                        related_name='complaints')  # Optional link to service request
