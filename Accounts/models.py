...
class Franchisee(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='franchisee')
    custom_id = models.CharField(
        max_length=10, unique=True, editable=False, blank=True)

    about = models.TextField()
    profile_image = models.ImageField(upload_to='f-profile_images/', null=True,
                                      blank=True, validators=[validate_file_size])  # Profile image field
    ...
    type = models.ForeignKey(
        Franchise_Type, on_delete=models.CASCADE, related_name='franchisee_type')
    ...
    verificationid_number = models.CharField(
        max_length=50, blank=True, null=True)  # ID number field
...
class Dealer(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='dealer')
    custom_id = models.CharField(
        max_length=10, unique=True, editable=False, blank=True)  # Custom ID field

    about = models.TextField()
    profile_image = models.ImageField(upload_to='d-profile-images/', null=True,
                                      blank=True, validators=[validate_file_size])  # Profile image field
    service_providers = models.IntegerField(blank=True, null=True)
    ...
    id_copy = models.FileField(
        upload_to='id-dealer/', blank=True, null=True, validators=[validate_file_size])

    def save(self, *args, **kwargs):
        if not self.custom_id:
            # Generate the custom ID format
            # Franchisee ID with prefix FR
            franchisee_id = f'FR{self.franchisee.id}'

            # Combine to form the custom ID
            # Format: D{id}FR{id}
            self.custom_id = f'D{self.id}{franchisee_id}'
...
class ServiceProvider(models.Model):
    ...
    custom_id = models.CharField(
        max_length=20, unique=True, editable=False, blank=True)  # Custom ID field
    ...
    profile_image = models.ImageField(
        upload_to='s-profile-images/', null=True, blank=True, validators=[validate_file_size])
    ...
    address_proof_document = models.CharField(
        max_length=255, blank=True, null=True)
    id_number = models.CharField(
        max_length=50, blank=True, null=True)  # ID number field
    address_proof_file = models.FileField(upload_to='id-service-pro/', blank=True,
                                          null=True, validators=[validate_file_size])  # File upload for address proof
    payout_required = models.CharField(
        max_length=10, choices=PAYOUT_FREQUENCY_CHOICES)  # Payout frequency field
    status = models.CharField(max_length=10, choices=[(
        'Active', 'Active'), ('Inactive', 'Inactive')])
    verification_by_dealer = models.CharField(
    ...
    def save(self, *args, **kwargs):
        if not self.custom_id:
            ...
            # Combine to form the custom ID
            # Format: SP{id}D{id}FR{id}
            self.custom_id = f'SP{self.user.id}{dealer_id}{franchisee_id}'

        super(ServiceProvider, self).save(*args, **kwargs)

    def __str__(self):
        # return self.custom_id for pushing
...
class Customer(models.Model):
    ...
    profile_image = models.ImageField(
        upload_to='c-profile-images/', null=True, blank=True, validators=[validate_file_size])
    date_of_birth = models.DateField(null=True, blank=True)
    gender = models.CharField(max_length=1, choices=GENDER_CHOICES)
...
class OTP(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='otp_received_user')
...
class Service_Type(models.Model):
    ...
    curreny = models.CharField(
        max_length=10, null=True, blank=True, default="INR")
...
class Subcategory(models.Model):
    ...
    service_type = models.ForeignKey(
        Service_Type, on_delete=models.PROTECT, related_name='service_type')
    collar = models.ForeignKey(
        Collar, on_delete=models.PROTECT, related_name='collar')
    status = models.CharField(max_length=10, choices=[(
        'Active', 'Active'), ('Inactive', 'Inactive')])
...
class ServiceRegister(models.Model):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.PROTECT, related_name='serviceregister_category')
    # collar = models.ForeignKey(Collar, on_delete=models.PROTECT,related_name='service_register_collars')
    # amount = models.DecimalField(max_digits=10, decimal_places=2)
    subcategory = models.ForeignKey(
        Subcategory, on_delete=models.PROTECT, related_name='serviceregister_subcategory')
    license = models.FileField(upload_to='service-license/',
...
class PaymentRequest(models.Model):
    service_provider = models.ForeignKey(
        ServiceProvider, on_delete=models.PROTECT, related_name='from_paymentrequest')
    dealer = models.ForeignKey(
        Dealer, on_delete=models.PROTECT, related_name='to_paymentrequest')
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    description = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
    email = models.EmailField()
    country_code = models.ForeignKey(
        Country_Codes, max_length=25, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    supporting_documents = models.FileField(
        upload_to='payment-request/', blank=True, null=True, validators=[validate_file_size])
...
class CustomerReview(models.Model):
    ...
    customer = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name='from_review')
    # The service provider being reviewed
    service_provider = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name='to_review')
    # Rating from 1 to 5 stars
    rating = models.IntegerField(choices=RATING_CHOICES)
    image = models.ImageField(
        upload_to='reviews/', null=True, blank=True, validators=[validate_file_size])
    comment = models.TextField(blank=True, null=True)  # Optional comment
    created_at = models.DateTimeField(
        auto_now_add=True)  # Auto-set the review date
...
class ServiceRequest(models.Model):
    ...
    booking_id = models.CharField(
        max_length=20, unique=True, editable=False, blank=True)  # Custom ID field
    customer = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name='to_servicerequest')
    service_provider = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name='from_servicerequest')
    service = models.ForeignKey(
        ServiceRegister, on_delete=models.PROTECT, related_name='servicerequest')
    work_status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='pending')
    acceptance_status = models.CharField(max_length=20, choices=[(
        'accept', 'accept'), ('decline', 'decline'), ('pending', 'pending')], default='pending')
    rescheduled = models.BooleanField(default=False)  # new field
    ...
    def __str__(self):
        # return f"Request by {self.customer.full_name} for {self.service.title} ({self.acceptance_status})" for pushing
    ...
    def clean(self):
        ...
        if self.availability_from >= self.availability_to:
            raise ValidationError(
                'Availability "from" time must be before "to" time.')
...
class Invoice(models.Model):
    ...
    service_request = models.ForeignKey(
        ServiceRequest, on_delete=models.SET_NULL, null=True, blank=True, related_name='invoices')

    # Sender (user who is paying) and receiver (user receiving payment)
    sender = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='sent_payment')
    receiver = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='received_payment')

    quantity = models.IntegerField(null=True, blank=True)
    price = models.DecimalField(max_digits=10, decimal_places=2)
    total_amount = models.DecimalField(max_digits=10, decimal_places=2)
    payment_status = models.CharField(max_length=20, choices=[('pending', 'Pending'), (
        'paid', 'Paid'), ('cancelled', 'Cancelled'), ('partial_paid', 'Partial paid')], default='pending')
...
class Complaint(models.Model):
    ...
    customer = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='sent_compliant')
    service_provider = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='received_compliant')
    service_request = models.ForeignKey(ServiceRequest, on_delete=models.SET_NULL, null=True,
                                        blank=True, related_name='complaints')  # Optional link to service request
    subject = models.CharField(max_length=255)
    description = models.TextField()
    images = models.ImageField(
        upload_to='complaint/', null=True, blank=True, validators=[validate_file_size])
...
class DeclineServiceModel(models.Model):
    ...
    images = models.ImageField(
        upload_to='decline/', null=True, blank=True, validators=[validate_file_size])
